...
{ //Extends Editor required
    ...
    void DuplicateAtIndex(int index)
    {
    ...
    public override void OnInspectorGUI() //Must be overwritten, this is called everytime the mouse moves over the inspector
    {
        ...
        for (int i = 0; i < wpArrayCount.intValue; i++)
        {
            ...
            if (!GUI.changed)
            {
                GUI.enabled = (GetWaypointAt(i)) && initialEnabledSetting;      //Reset the state of the UI! if you leave it disabled, it will disable everything that follows
                if (GUILayout.Button("d", GUILayout.Width(20f)))  //A button of width 20
                    DuplicateAtIndex(i);                     //Calls duplicate function on current for loop index (i)
                GUI.enabled = initialEnabledSetting;      //Reset the state of the UI! if you leave it disabled, it will disable everything that follows
            }
    ...
    private void OnSceneGUI() //Monobehaviour editor function
    {
        ...
        if (mc.waypoints.Length > 1)
        {
            ...
            for (int i = 0; i < mc.waypoints.Length; i++)                                              //Length minus one, because the last one doesnt have a "next"
            {
                if (mc.waypoints[i] && currentWaypoint != -1)   // Checks if there's other waypoint not null
                {
                    nextWaypoint = i;
                    lastOf = i;
                }
                if (mc.waypoints[i] && currentWaypoint == -1)   // Check for the very first waypoint not null 1 time
                {
                    firstOf = i;
                    lastOf = i;
                    currentWaypoint = i;
                }
                if (currentWaypoint != -1 && nextWaypoint != -1)    // If there's at least two waypoints
                {
                    Handles.DrawLine(mc.waypoints[currentWaypoint].position, mc.waypoints[nextWaypoint].position);               //Draw a line from currentWaypoint to nextWaypoint
                    currentWaypoint = nextWaypoint;
                    nextWaypoint = -1;
                }
            }
            if (firstOf != -1)
                Handles.DrawLine(mc.waypoints[lastOf].position, mc.waypoints[firstOf].position);     //Draw line from last element back to first element
